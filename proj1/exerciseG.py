...
r2_ols_train = np.zeros((len(degree), folds))
r2_ols_val = np.zeros((len(degree), folds))
...
r2_ridge_train = np.zeros((len(degree), len(lambda_values), folds))
r2_ridge_val = np.zeros((len(degree), len(lambda_values), folds))
...
r2_lasso_train = np.zeros((len(degree), len(lambda_values), folds))
r2_lasso_val = np.zeros((len(degree), len(lambda_values), folds))
...
for train_inds, val_inds in kfold.split(X_train):
    ...
    for deg in degree:
        ...
        r2_ols_train[deg - 1, fold_num] = r2_score(y_train, y_pred_OLS_train)
        r2_ols_val[deg - 1, fold_num] = r2_score(y_val, y_pred_OLS_val)
        ...
        for lmb in lambda_values:
            ...
            mse_ridge_train[deg - 1, lmb_num, fold_num] = mean_squared_error(
                y_train, y_pred_ridge_train
            )
            mse_ridge_val[deg - 1, lmb_num, fold_num] = mean_squared_error(
                y_val, y_pred_ridge_val
            )

            r2_ridge_train[deg - 1, lmb_num, fold_num] = r2_score(
                y_train, y_pred_ridge_train
            )
            r2_ridge_val[deg - 1, lmb_num, fold_num] = r2_score(y_val, y_pred_ridge_val)
            ...
            mse_lasso_train[deg - 1, lmb_num, fold_num] = mean_squared_error(
                y_train, y_pred_lasso_train
            )
            mse_lasso_val[deg - 1, lmb_num, fold_num] = mean_squared_error(
                y_val, y_pred_lasso_val
            )

            r2_lasso_train[deg - 1, lmb_num, fold_num] = r2_score(
                y_train, y_pred_lasso_train
            )
            r2_lasso_val[deg - 1, lmb_num, fold_num] = r2_score(y_val, y_pred_lasso_val)
...
avg_r2_ols_train = np.mean(r2_ols_train, axis=1)
avg_r2_ols_val = np.mean(r2_ols_val, axis=1)
...
avg_r2_ridge_train = np.mean(r2_ridge_train, axis=2)
avg_r2_ridge_val = np.mean(r2_ridge_val, axis=2)
...
avg_r2_lasso_train = np.mean(r2_lasso_train, axis=2)
avg_r2_lasso_val = np.mean(r2_lasso_val, axis=2)
...
print(
    "Degree "
    + str(best_ols_degree_train)
    + " gives MSE "
    + str(avg_mse_ols_train[min_ols_index_train])
    + " for OLS on training data"
)
...
print(
    "Degree "
    + str(best_ols_degree_val)
    + " gives MSE "
    + str(avg_mse_ols_val[min_ols_index_val])
    + " for OLS on val data"
)

print("Train cases for Ridge")
min_ridge_index_train = np.unravel_index(
    np.argmin(avg_mse_ridge_train), avg_mse_ridge_train.shape
)
best_ridge_degree_train = degree[min_ridge_index_train[0]]
best_ridge_lambda_train = lambda_values[min_ridge_index_train[1]]
print(
    "Degree "
    + str(best_ridge_degree_train)
    + " and lambda "
    + str(best_ridge_lambda_train)
    + " gives MSE "
    + str(avg_mse_ridge_train[min_ridge_index_train[0], min_ridge_index_train[1]])
    + " for Ridge on training data"
)

print("Val cases for Ridge")
min_ridge_index_val = np.unravel_index(
    np.argmin(avg_mse_ridge_val), avg_mse_ridge_val.shape
)
best_ridge_degree_val = degree[min_ridge_index_val[0]]
best_ridge_lambda_val = lambda_values[min_ridge_index_val[1]]
print(
    "Degree "
    + str(best_ridge_degree_val)
    + " and lambda "
    + str(best_ridge_lambda_val)
    + " gives MSE "
    + str(avg_mse_ridge_val[min_ridge_index_val[0], min_ridge_index_val[1]])
    + " for Ridge on val data"
)

print("Train cases for Lasso")
min_lasso_index_train = np.unravel_index(
    np.argmin(avg_mse_lasso_train), avg_mse_lasso_train.shape
)
best_lasso_degree_train = degree[min_lasso_index_train[0]]
best_lasso_lambda_train = lambda_values[min_lasso_index_train[1]]
print(
    "Degree "
    + str(best_lasso_degree_train)
    + " and lambda "
    + str(best_lasso_lambda_train)
    + " gives MSE "
    + str(avg_mse_lasso_train[min_lasso_index_train[0], min_lasso_index_train[1]])
    + " for Lasso on training data"
)

print("Val cases for Lasso")
min_lasso_index_val = np.unravel_index(
    np.argmin(avg_mse_lasso_val), avg_mse_lasso_val.shape
)
best_lasso_degree_val = degree[min_lasso_index_val[0]]
best_lasso_lambda_val = lambda_values[min_lasso_index_val[1]]
print(
    "Degree "
    + str(best_lasso_degree_val)
    + " and lambda "
    + str(best_lasso_lambda_val)
    + " gives MSE "
    + str(avg_mse_lasso_val[min_lasso_index_val[0], min_lasso_index_val[1]])
    + " for Lasso on val data"
)
...
beta_ridge = (
    np.linalg.inv(
        ...
        + best_ridge_lambda_val * np.eye(x_train_poly.shape[1])
    )
    @ x_train_poly.T
    @ Y_train
)
...
print_metrics(
    ...
    avg_bias_ols_train,
    avg_bias_ols_val,
    avg_variance_ols_train,
    avg_variance_ols_val,
    degree,
    None,
)
...
print_metrics(
    ...
    avg_mse_ridge_train,
    avg_mse_ridge_val,
    avg_bias_ridge_train,
    avg_bias_ridge_val,
    avg_variance_ridge_train,
    avg_variance_ridge_val,
    degree,
    lambda_values,
)
...
print_metrics(
    ...
    avg_mse_lasso_train,
    avg_mse_lasso_val,
    avg_bias_lasso_train,
    avg_bias_lasso_val,
    avg_variance_lasso_train,
    avg_variance_lasso_val,
    degree,
    lambda_values,
)
# Plot MSE
...
plt.plot(degree, avg_mse_ols_val, marker="o", label="Val MSE", color="orange")
...
for i, lmb in enumerate(lambda_values):
    plt.plot(
        degree, avg_mse_ridge_train[:, i], marker="o", label=f"Train MSE (λ={lmb})"
    )
    plt.plot(
        degree,
        avg_mse_ridge_val[:, i],
...
for i, lmb in enumerate(lambda_values):
    plt.plot(
        degree, avg_mse_lasso_train[:, i], marker="o", label=f"Train MSE (λ={lmb})"
    )
    plt.plot(
        degree,
        avg_mse_lasso_val[:, i],
...
plt.subplot(3, 2, 1)
plt.plot(degree, avg_r2_ols_train, marker="o", label="Train r2", color="blue")
plt.plot(degree, avg_r2_ols_val, marker="o", label="Test r2", color="orange")
plt.title("Avg. OLS Regression r2")
plt.xlabel("Polynomial Degree")
plt.ylabel(" Avg. r2")
plt.xticks(degree)
plt.legend()
...
plt.subplot(3, 2, 2)
for i, lmb in enumerate(lambda_values):
    plt.plot(degree, avg_r2_ridge_train[:, i], marker="o", label=f"Train r2 (λ={lmb})")
    plt.plot(
        degree,
        avg_r2_ridge_val[:, i],
        marker="o",
        linestyle="--",
        label=f"Val r2 (λ={lmb})",
    )
plt.title("Avg. Ridge Regression r2")
plt.xlabel("Polynomial Degree")
plt.ylabel("Avg. r2")
plt.xticks(degree)
plt.legend()
...
plt.subplot(3, 2, 3)
for i, lmb in enumerate(lambda_values):
    plt.plot(degree, avg_r2_lasso_train[:, i], marker="o", label=f"Train r2 (λ={lmb})")
    plt.plot(
        degree,
        avg_r2_lasso_val[:, i],
        marker="o",
        linestyle="--",
        label=f"Test r2 (λ={lmb})",
    )
plt.title("Avg. Lasso Regression r2")
plt.xlabel("Polynomial Degree")
plt.ylabel("Avg. r2")
plt.xticks(degree)
plt.legend()
